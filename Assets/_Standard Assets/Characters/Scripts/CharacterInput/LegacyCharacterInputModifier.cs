...
namespace StandardAssets.Characters.CharacterInput
{
	...
	/// </summary>
	[Serializable]
	public class LegacyCharacterInputModifier
	{
		/// <summary>
		...
		/// <summary>
		/// If angle from tail to head is less than this and the direction from tail to head changed, then assume the head went past the tail
		/// </summary>
		...
		/// <summary>
		/// Enable the modifier? Smooths the input movement when rotating in fast circles. This makes the character run
		/// in a circle, instead of turning around on the spot.
		/// </summary>
		[SerializeField, Tooltip("Enable the modifier? Smooths the input movement when rotating in fast circles. " +
		                         "This makes the character run in a circle, instead of turning around on the spot.")]
		private bool enableModifier;
		...
		[SerializeField, Tooltip("The character's transform.")]
		private Transform debugCharacterTransform;
		...
		public bool enabled
		{
			get { return enableModifier; }
			set { enableModifier = value; }
		}
		
		/// <summary>
		/// Update the move input, if needed.
		/// </summary>
		public void UpdateMoveInput(ref Vector2 moveInput)
		{
			if (!enabled)
			{
				return;
		...
		private void DebugUpdate()
		{
			...
			if (debugCharacterTransform == null)
			{
				return;
			}
			...
			if (debugShowHead)
			{
				...
				{
					DebugDrawInput(debugHeadInput, offsetY, debugHeadColor, duration);
				}
			}
			...
			{
				DebugDrawInput(new Vector2(debugCharacterTransform.forward.x, debugCharacterTransform.forward.z), 
		                            ...
		                            float scale = 1.0f)
		{
			...
			Vector3 point = debugCharacterTransform.position + new Vector3(0.0f, offsetY, 0.0f);
			if (debugCamera != null)
			{
